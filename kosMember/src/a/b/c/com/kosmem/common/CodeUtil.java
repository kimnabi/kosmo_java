...
public abstract class CodeUtil {
	...
	// 검색어필터 라벨
	public static final String[] searchFilter_lavel = {"ALL", "MNUM", "MNAME", "MID"};
	public static final String[] searchFilter_value = {"01", "02", "03", "04"};
	...
	// 검색어필터의값
	public static String dBsearchFilter_value(String s){
		System.out.println(s);
		String h = "";
		String ss[] = s.split(",");
		System.out.println(ss.length);
		for (int i=0; i < ss.length; i++){			
			System.out.println(ss[i]);
			System.out.println(dBsearchFilter_lavel(ss[i]));
			h += hobby(ss[i]) + " ";
		}		
		return h;
	}
	...
	//검색어필터의레벨
	public static String dBsearchFilter_lavel(String s) {
		
		String h = "";				
		if (s !=null && s.length() > 0){							
			for (int i=1; i < searchFilter_lavel.length; i++) {	
				if (("0" + i).equals(s)) {
					h = CodeUtil.searchFilter_lavel[i-1];
				}
			}			
		}		
		return h;
