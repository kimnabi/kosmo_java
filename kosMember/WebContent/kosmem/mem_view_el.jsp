...
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
 ...
 logger.info("mem_view_el.jsp 블록 진입  >>> ");
 ...
 logger.info("mem_view_el.jsp 블록 진입 222222222222 >>> ");
	String mhp = "";
	String memail = "";
	String mgender = "";
	String mhobby = "";
...
<tr>
	<td>회원번호</td>
	<td><input type="text" name="mnum"  id="mnum" readonly  value="${mvo.mnum}"/></td>
</tr>
<tr>
	<td>이름</td>
	<td><input type="text" name="mname" id="mname" value="${mvo.mname}"/></td>
</tr>
<tr>
	<td>아이디</td>
	<td>	
		<input type="text" name="mid" mid="mid" placeholder="아이디체크"  value="${mvo.mid}"/>
...
<tr>
	<td>패스워드</td>
	<td>
		<input type="text" name="mpw" id="mpw" style="width:100px" value="${mvo.mpw}"/><br>
...
<tr>
	...
	<td>
		<select name="mhp" id="mhp" style="width:50px;">
		     <c:if test="${'010' eq hpstr[0]}" >
        		<option value="010" selected="${selected}" >010</option>
        	</c:if>
        	<c:if test="${'010' ne hpstr[0]}" >
        		<option value="010" >010</option>
        	</c:if>
		     <c:if test="${'011' eq hpstr[1]}" >
        		<option value="010" selected="${selected}" >011</option>
        	</c:if>
        	<c:if test="${'011' ne hpstr[1]}" >
        		<option value="011" >011</option>
        	</c:if>
        	  <c:if test="${'016' eq hpstr[2]}" >
        		<option value="016" selected="${selected}" >016</option>
        	</c:if>
        	 <c:if test="${'016' ne hpstr[2]}" >
        		<option value="016" >016</option>
        	</c:if>
        	 <c:if test="${'017' eq hpstr[3]}" >
        		<option value="017" selected="${selected}" >017</option>
        	 </c:if>
        	 <c:if test="${'017' ne hpstr[3]}" >
...
<tr>
	<td>성별</td>
	<td>
			 <c:if test="${'01' eq mvo.mgender}" >
        		<input type="radio" name="mgender" value="01" checked="checked"/>여자
        	 </c:if>
        	 <c:if test="${'01' ne mvo.mgender}" >
        		<input type="radio" name="mgender" value="01" />여자
        	</c:if> 
			 <c:if test="${'02' eq mvo.mgender}" >
        		<input type="radio" name="mgender" value="02" checked="checked"/>남자
        	 </c:if>
        	 <c:if test="${'02' ne mvo.mgender}" >
